    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_SPACE:
                ...
                pairing.append(("ucs",current_time))
            if(event.key==pygame.K_b):
                current_time=pygame.time.get_ticks()
                bell.play()
                pairing.append(("cs",current_time))
                cs_time=[t for k, t in pairing if k=="cs"]
                ucs_time=[t for k, t in pairing if k=="ucs"]
                recent_pairing=[(cs,ucs)
                                for cs in cs_time
                                for ucs in ucs_time
                                if 0<(ucs-cs)<=1000]
                if len(recent_pairing)>=condition_threshold:
                    cs_learned=True
                if cs_learned:
                    for circle in circles:
                        circle.reactivate(current_time)
